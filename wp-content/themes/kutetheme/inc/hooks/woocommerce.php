        ...
        if ( ! WC()->cart->is_empty() ) : ?>
            <div class="cart-block">
                <div class="cart-block-content">
                    <h5 class="cart-title"><?php echo sprintf (_n( '%d sản phẩm trong giỏ', '%d sản phẩm trong giỏ hàng', WC()->cart->cart_contents_count, 'kutetheme' ), WC()->cart->cart_contents_count ); ?></h5>
        ...
        <?php if( kt_is_wc() ):?>
            ...
            <?php if( is_user_logged_in()): ?>
                ...
                <li class="menu-item menu-item-has-children">
                    ...
                    <?php endif;?>
                    <ul class="sub-menu">
                        <li><a href="<?php echo esc_url( $myaccount_link );?>"><?php _e('Quản lý tài khoản','kutetheme');?></a></li>
                        <?php 
                        if( function_exists( 'YITH_WCWL' ) ):
                            $wishlist_url = YITH_WCWL()->get_wishlist_url();
                        ?>
                        <li><a href="<?php echo esc_attr( $wishlist_url );?>"><?php _e('Danh sách yêu thích','kutetheme');?></a></li>
                        <?php endif;?>
                        <?php if(defined( 'YITH_WOOCOMPARE' )): 
                            global $yith_woocompare; 
                            $count = count($yith_woocompare->obj->products_list); ?>
                        <li><a href="#" class="yith-woocompare-open"><?php esc_html_e( "So sánh sản phẩm", 'kutetheme') ?><span>(<?php echo esc_attr( $count ); ?>)</span></a></li>
                        <?php endif; ?>
                        <?php if(isset($logout_url) && $logout_url):?>
                        <li><a href="<?php echo esc_url( $logout_url );?>"><?php _e('Đăng xuất','kutetheme');?></a></li>
                        <?php endif;?>
                    </ul>
                </li>
            <?php else:?>
                <li class="menu-item login-item"><a href="<?php echo esc_url( $myaccount_link );?>"><?php _e('Đăng nhập/đăng ký','kutetheme');?></a></li>
            <?php endif;?>
        <?php endif;?>
   <?php
   }
